...
local PAD_ITEM = "edoras_horse:saddle_pad"
-- White mask painted over the body's back + upper flanks; colorised per dye at
-- equip time (see refresh_textures / horse:pad_color). Generated by tools/make_pad.py.
local PAD_MASK = "edoras_horse_saddlepad.png"
...
local function is_pad_item (stack)
	return stack:get_name () == PAD_ITEM
end

-- The saddle pad stores its dyed colour in item metadata ("pad_color", a #rrggbb
-- string). Read it back from an itemstring; nil if absent/unset.
local function pad_color_of (itemstring)
	if not itemstring or itemstring == "" then return nil end
	local meta = ItemStack (itemstring):get_meta ()
	local c = meta:get_string ("pad_color")
...
-- Rebuild the 3-layer texture set (chest, fur+pad+armor, saddle).
-- Layer 2 is the coat, then the dyed saddle pad (a colorised mask over the back
-- and upper flanks), then any armor overlay on top; layer 3 exposes the saddle
...
function horse:refresh_textures ()
	...
	self._naked_fur = fur
	local body = fur
	-- Saddle pad: drape a colorised mask over the coat (under the saddle/armor).
	local padcol = pad_color_of (self._pad)
	if padcol then
		body = body .. "^(" .. PAD_MASK .. "^[multiply:" .. padcol .. ")"
	end
	if self._armor and self._armor ~= "" then
		...
		if def and def._horse_overlay_image then
			body = body .. "^" .. def._horse_overlay_image
		end
	end
...
-- Saddle pad: a dyed cloth that drapes the back and upper flanks (cosmetic).
-- The whole itemstring is stored (it carries the colour in metadata) so the exact
-- dyed pad is returned on removal/death. Persists via mcl_mobs staticdata.
function horse:has_pad ()
	return (self._pad or "") ~= ""
end

-- Only ever called on an already-tamed horse (callers gate on self.tamed).
function horse:apply_pad (stack)
	self._pad = stack:to_string ()
	self:refresh_textures ()
	core.sound_play ("mcl_armor_equip_leather", {
		gain = 0.5, max_hear_distance = 8, pos = self.object:get_pos (),
	}, true)
...
function horse:inv_formspec (inv_name, has_bag)
	...
	-- Pad slot (main index 4). The saddlebag storage grid occupies x>=2.5 when a
	-- bag is equipped, so park the Pad slot beside the Saddle (x=1.9) normally and
	-- drop it below the Bag (x=0.6) when the storage panel is showing.
	local pad_x, pad_y = (has_bag and 0.6 or 1.9), (has_bag and 5.9 or 1.4)
	fs[#fs + 1] = string.format ("label[%g,%g;%s]", pad_x, pad_y - 0.3,
		core.formspec_escape (S("Pad")))
	fs[#fs + 1] = slot_bg (pad_x, pad_y, 1, 1)
	fs[#fs + 1] = string.format ("list[%s;main;%g,%g;1,1;3]", list, pad_x, pad_y)
	if has_bag then
	...
	-- With a bag equipped the Pad slot sits below the Bag (y=5.9), so push the whole
	-- player inventory (and the window) down to clear it; otherwise keep it compact.
	local inv_y = has_bag and 7.1 or 5.8
	local hot_y = inv_y + 3.75            -- 3 rows at 1.25 spacing + a 1.25 gap
	fs[2] = string.format ("size[12.2,%g]", hot_y + 1.75)
	fs[#fs + 1] = slot_bg (0.6, inv_y, 9, 3)
	fs[#fs + 1] = string.format ("list[current_player;main;0.6,%g;9,3;9]", inv_y)
	fs[#fs + 1] = slot_bg (0.6, hot_y, 9, 1)
	fs[#fs + 1] = string.format ("list[current_player;main;0.6,%g;9,1;0]", hot_y)
...
function horse:open_inventory (clicker)
	...
	local inv = core.create_detached_inventory (inv_name, {
		...
		allow_put = function (inv, listname, index, stack)
			...
			elseif index == PAD_SLOT then
				return is_pad_item (stack) and 1 or 0
			end
			return 0
		end,
	...
	inv:set_stack ("main", PAD_SLOT, ItemStack (self._pad))
...
function horse:on_rightclick (clicker)
	...
	-- Lay a dyed saddle pad on a tamed horse (remove it via the gear panel).
	if is_pad_item (stack) and not self:has_pad () and self.tamed
		and not self.child then
		self:apply_pad (stack)
		if not creative then
			stack:take_item ()
			clicker:set_wielded_item (stack)
		end
...
function horse:on_die ()
	...
	if self:has_pad () then
		core.add_item (pos, self._pad)
	end
	if self._saddlebag then
		for _, s in ipairs (self._saddlebag) do
			if s ~= "" then
...
function horse:after_activate ()
	...
	self:init_genetics ()
	self:init_taming ()
	if self.saddle == "yes" or (self._armor and self._armor ~= "") or self:has_bag ()
		or self:has_pad () then
		self:refresh_textures ()
	end
	if self._armor and self._armor ~= "" then
...
------------------------------------------------------------------------
-- Saddle pad: a dyed cloth laid on a tamed horse (under the saddle) that drapes
-- the back and upper flanks. Crafted from 3 carpet + 1 dye, or + 2 dyes to blend
-- a colour (e.g. red + white = pink). The chosen colour is averaged from the
-- dye(s) and stored in the item's metadata, so it survives crafting -> equip ->
-- removal and renders as a `^[multiply` tint over the pad mask (refresh_textures).
------------------------------------------------------------------------

core.register_craftitem(PAD_ITEM, {
	description = S("Saddle Pad"),
	_doc_items_longdesc = S("A dyed cloth pad. Lay it on a tamed horse (it sits "
		.. "under the saddle) for a band of colour down its back and sides. Craft "
		.. "with 3 carpet and a dye -- or two dyes to blend a colour."),
	inventory_image = "edoras_horse_saddlepad_item.png",
	stack_max = 1,
})

-- 3 carpet + 1 or 2 dyes (shapeless). The colour is resolved in the craft hooks
-- below, so the recipe output itself is the plain (white) pad.
core.register_craft({
	type = "shapeless",
	output = PAD_ITEM,
	recipe = {"group:carpet", "group:carpet", "group:carpet", "group:dye"},
})
core.register_craft({
	type = "shapeless",
	output = PAD_ITEM,
	recipe = {"group:carpet", "group:carpet", "group:carpet", "group:dye", "group:dye"},
})

-- Map each dye item -> its #rrggbb (from mcl_dyes). Built once at load; empty (so
-- pads stay white) if mcl_dyes is somehow absent.
local dye_rgb = {}
if mcl_dyes and mcl_dyes.colors then
	for k, v in pairs (mcl_dyes.colors) do
		if v.rgb then dye_rgb["mcl_dyes:" .. k] = v.rgb end
	end
end
...
-- Average the colours of all dyes in the craft grid; nil if none recognised.
local function pad_color_from_grid (grid)
	local r, g, b, n = 0, 0, 0, 0
	for _, st in ipairs (grid) do
		local hex = dye_rgb[st:get_name ()]
		if hex then
			local rr, gg, bb = hex_to_rgb (hex)
			r, g, b, n = r + rr, g + gg, b + bb, n + 1
		end
	end
	if n == 0 then return nil end
	return string.format ("#%02x%02x%02x", math.floor (r / n + 0.5),
		math.floor (g / n + 0.5), math.floor (b / n + 0.5))
end

-- Stamp the blended colour onto a freshly-crafted pad (called from both the real
-- craft and the recipe preview so the grid shows the right colour). Returns the
-- coloured stack, or nil to leave the craft result unchanged.
local function colorize_crafted_pad (itemstack, grid)
	if itemstack:get_name () ~= PAD_ITEM then return nil end
	local hex = pad_color_from_grid (grid)
	if not hex then return nil end
	local meta = itemstack:get_meta ()
	meta:set_string ("pad_color", hex)
	meta:set_string ("inventory_image",
		"edoras_horse_saddlepad_item.png^[multiply:" .. hex)
	return itemstack
end

core.register_on_craft (function (itemstack, player, old_craft_grid, craft_inv)
	return colorize_crafted_pad (itemstack, old_craft_grid)
end)
